      ...
      <v-layout row wrap>
        ...
        <v-flex v-for="category in categories" xs6 :key="category.id">
          <v-card :to="'/category/' + category.slug">
            <v-card-media v-if="category.image" :src="getImage('/categories/' + category.image)" height="150px">
      ...
      go(){
        ...
        this.axios.get(url)
          .then((response) => {
            ...
            // jumlah halaman di dapat dari mete endpoint categories
            this.categories = categories
            // daftar category dari endpoint categories
          })
          .catch((error) => {
